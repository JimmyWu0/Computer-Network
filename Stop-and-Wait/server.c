...
void sendFile(FILE *fd) {
    ...
    while (current < filesize) {
        ...
        if(current+1024>=filesize){
            ...
            send.header.isLast=1;
        }else{
            send.header.size=1024;
            send.header.isLast=0;
        }
        ...
        int numbytes;
		numbytes=sendto(sockfd, &send, sizeof(send), 0,(struct sockaddr *)&clientInfo, addrlen);  //!!
        ...
        struct timeval tv;
			tv.tv_usec=1*1000; //microseconds   待調整
			tv.tv_sec=0;
			fd_set rdfds;
			int re;
			FD_ZERO(&rdfds); FD_SET(sockfd,&rdfds);
			re=select(sockfd+1,&rdfds,NULL,NULL,&tv); //sockfd+1 
			if(re==0){
				usleep(99*1000);
				...
				recvfrom(sockfd, &recv, sizeof(recv), 0, (struct sockaddr *)&clientInfo, &addrlen);
			}
        ...
        int result = poll(pollFds, 1, TIMEOUT);
        ...
        send.header.seq=current;  //may have problem
    }
}
